...
export default async function handler(req, res) {
  ...
  if (req.method === "POST") {
    ...
    const { mes_ano, ref_cod, meta_turno, num_turnos } = getBody(req);
  ...
  if (req.method === "DELETE") {
    if (u.perfil !== "admin") return res.status(403).json({ error: "Acesso negado" });
    const { mes_ano, ref_cod } = getBody(req);
    try {
      await sql`UPDATE programa SET ativo = false WHERE mes_ano = ${mes_ano} AND ref_cod = ${ref_cod}`;
      return res.status(200).json({ ok: true });
    } catch (e) {
      return res.status(500).json({ error: e.message });
    }
